...
class Tests(unittest.TestCase):
    ...
    def init_object(self, eigenvector_method, max_pi_err):
    ...
    def test_g1_torch_eq_pi(self):
        t_val = torch.tensor([[0, 0.5], [0.5, 0]], dtype=torch.float)
        self.init_object(eigenvector_method=EigenvectorMethod.power_iteration, max_pi_err=0.0001)
        pi_rbc = self.rbc_spbc(g1, t_val)[0]
        self.init_object(eigenvector_method=EigenvectorMethod.torch_eig, max_pi_err=0.0001)
        eig_rbc = self.rbc_spbc(g1, t_val)[0]
    ...
    def test_g2_pi_spbc(self):
        ...
        t_val = torch.full(size=(7, 7), fill_value=0.5, dtype=torch.float)
        t_val[torch.eye(7).byte()] = 0.0
        expected_rbc = nx.betweenness_centrality(g2, normalized=False, endpoints=True)
        actual_rbc, nodes_map_g = self.rbc_spbc(g2, t_val)
        for key, val in expected_rbc.items():
            self.assertTrue(actual_rbc[nodes_map_g[key]] == val)
    ...
    def test_g2_torch_eq_pi(self):
        ...
        t_val[torch.eye(7).byte()] = 0
        self.init_object(eigenvector_method=EigenvectorMethod.power_iteration, max_pi_err=0.0001)
        pi_rbc = self.rbc_spbc(g2, t_val)[0]
        self.init_object(eigenvector_method=EigenvectorMethod.torch_eig, max_pi_err=0.0001)
        eig_rbc = self.rbc_spbc(g2, t_val)[0]
        self.assertTrue(all(pi_rbc == eig_rbc))

    def test_g3_pi_spbc(self):
        self.init_object(eigenvector_method=EigenvectorMethod.power_iteration, max_pi_err=0.00001)
        t_val = torch.full(size=(3, 3), fill_value=0.5, dtype=torch.float)
        t_val[torch.eye(3).byte()] = 0.0
        expected_rbc = nx.betweenness_centrality(g3, normalized=False, endpoints=True)
        actual_rbc, nodes_map_g = self.rbc_spbc(g3, t_val)
        for key, val in expected_rbc.items():
            self.assertTrue(actual_rbc[nodes_map_g[key]] == val)
    ...
    def test_g3_torch_eq_pi(self):
        ...
        t_val[torch.eye(3).byte()] = 0
        self.init_object(eigenvector_method=EigenvectorMethod.power_iteration, max_pi_err=0.0001)
        pi_rbc = self.rbc_spbc(g3, t_val)[0]
        self.init_object(eigenvector_method=EigenvectorMethod.torch_eig, max_pi_err=0.0001)
        eig_rbc = self.rbc_spbc(g3, t_val)[0]
        self.assertTrue(all(pi_rbc == eig_rbc))
    ...
    def test_g6_pi_spbc(self):
        t_val = torch.full(size=(4, 4), fill_value=0.5, dtype=torch.float)
        t_val[torch.eye(4).byte()] = 0
        self.init_object(eigenvector_method=EigenvectorMethod.power_iteration, max_pi_err=0.0001)
        pi_rbc = self.rbc_spbc(g6, t_val)[0]
        self.init_object(eigenvector_method=EigenvectorMethod.torch_eig, max_pi_err=0.0001)
        eig_rbc = self.rbc_spbc(g6, t_val)[0]
        self.assertTrue(all(pi_rbc == eig_rbc))
