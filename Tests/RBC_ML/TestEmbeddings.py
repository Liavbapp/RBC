...
def split_to_train_tests(p_man):
    ...
    train_data_lists = Rs[1:], Ts[1:], Gs[1:], path_params[1:]
    test_data_lists = [Rs[0]], [Ts[0]], [Gs[0]], [path_params[0]]
    return train_data_lists, test_data_lists


def generate_embeddings(train_graphs, test_graphs, train_seeds, test_seeds, prm_st):
...
def extract_info_from_path(paths, p_man):
    ...
    path_params = [{path: {EmbeddingPathParams.seed: random.randrange(1000)}} for path in paths]
...
def train_model(nn_model, train_info, p_man, optimizer, embeddings):
    train_Rs, train_Ts, train_Gs, seeds = train_info
    device = p_man.device
    dtype = p_man.dtype
