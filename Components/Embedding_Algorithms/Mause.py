...
class MUSAE(Estimator):
    ...
    def _create_documents(self, features):
        features_out = [
            TaggedDocument(words=[str(feat) for feat_elems in feature_set for feat in feat_elems], tags=[str(node)]) for
            node, feature_set in features.items()]
        return features_out
    ...
    def _create_base_docs(self):
        features_out = [TaggedDocument(words=[str(feature) for feature in features], tags=[str(node)]) for
                        node, features in self.features.items()]
        return features_out
